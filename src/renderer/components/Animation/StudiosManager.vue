<template>
  <div class="form-wrapper">
    ...
    <v-toolbar flat>
      <v-toolbar-title>Estúdios</v-toolbar-title>
    ...
    <v-data-table :headers="headers" :items="studios" class="elevation-1">
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? 'Novo estúdio' : 'Editar estúdio'
    }
  },
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      confirm('Tem certeza que deseja exluir esse estúdio?') &&
        this.$store.dispatch('deleteStudio', index)
    },
    ...
    save() {
      if (this.editedIndex > -1) {
        this.$store.dispatch('editStudio', {
          index: this.editedIndex,
          object: this.editedItem
        })
      } else {
        this.$store.dispatch('addStudio', this.editedItem)
      }
      this.close()
    },
